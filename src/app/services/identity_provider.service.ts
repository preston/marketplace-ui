...
export class IdentityProviderService extends BaseService {
    ...
    index() {
        let identity_providers = this.http.get(this.url(), this.options()).pipe(map(res => res.json()));
    ...
    create(identity_provider: IdentityProvider) {
        let obs = this.http.post(this.url(), { 'identity_provider': identity_provider }, this.options()).pipe(map(res => res.json()));
        return obs;
    }

    update(identity_provider: IdentityProvider) {
        let obs = this.http.put(this.url() + '/' + identity_provider.id, { 'identity_provider': identity_provider }, this.options()).pipe(map(res => res.json()));
        return obs;
    }

    delete(identity_provider: IdentityProvider) {
        let obs = this.http.delete(this.url() + '/' + identity_provider.id, this.options()).pipe(map(res => res.json()));
        return obs;
    }

    enable(identity_provider: IdentityProvider) {
        let obs = this.http.post(this.url() + '/' + identity_provider.id + '/enable', {}, this.options()).pipe(map(res => res.json()));
        return obs;
    }

    disable(identity_provider: IdentityProvider) {
        let obs = this.http.post(this.url() + '/' + identity_provider.id + '/disable', {}, this.options()).pipe(map(res => res.json()));
        return obs;
    }
